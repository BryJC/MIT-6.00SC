...
def bruteForceSearch(map_graph, start, end, maxTotalDist, maxDistOutdoors, visited = None, toPrint = False):    
    ...
    #here building == a tuple (child, weight, sub_weight) 
        ...
        if (str(destination) not in visited): 
            ...
            newPath = bruteForceSearch(map_graph, destination, end, maxTotalDist, maxDistOutdoors, visited, toPrint, counter += 1)
            ...
            if outdoor > maxDistOutdoors or currentPath > maxTotalDist:       #check to see if its too long
                visited.remove(str(destination))                              #necessary to avoid skipping previously checked paths...
                continue
                
                                                                         #if we made it thru AND it wasn't too big
...
#
def directedDFS(map_graph, start, end, maxTotalDist, maxDistOutdoors, visited = None, memo = None, toPrint = False, counter = 0):
    #TODO
    if visited == None:
        visited = []
    if memo == None:
    ...
    #here building == a tuple (child, weight, sub_weight) 
        ...
        if (str(destination) not in visited): 
            ...
            if (outdoor > maxDistOutdoors) or (currentPath > maxTotalDist):  #check to see if its too long
                visited.remove(str(destination))                             #necessary to avoid skipping previously checked paths...
                try:                                                        #needed because of earlier try/except clause:
                    del(memo[destination, end])                              #remove this segment so it cannot accidentally be called
                except:                                                      #as shortest path during subsequent calls
                    pass
                continue
                                                                        #if we made it thru AND it wasn't too big
            if bestPath == None or (currentPath < bestPath):             #check to see if first time to get this far on this level, 
